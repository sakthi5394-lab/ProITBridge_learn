...
includes a "inject synthetic noise" toggle — without it, uploading an already-clean
digit mostly demonstrates identity reconstruction, not denoising.
...
| `add_noise` | string (`"true"`/`"false"`) | no | Inject synthetic noise before prediction |
...
Expected for a clean-in upload on a denoising model — enable "inject synthetic
noise" in the UI to see actual denoising behavior.
